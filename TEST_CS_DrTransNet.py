...
class BasicBlock(torch.nn.Module):
    def __init__(self):
        ...
        self.net = TransNet(img_size=264,in_chans=1,win_size=6,token_mlp='ffn').cuda()
...
print('\n')
print("CS Sampling and Reconstruction by DrTransNet Start")
print('\n')
with torch.no_grad():
    for img_no in range(ImgNum):
        ...
        cv2.imwrite("%s_DrTransNet_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.png" % (resultName, cs_ratio, epoch_num, rec_PSNR, rec_SSIM), im_rec_rgb)
        del x_output
...
print('\n')
output_data = "CS ratio is %d, Avg PSNR/SSIM for %s is %.2f/%.4f/%.4f, Epoch number of model is %d \n" % (cs_ratio, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), np.mean(TIME_ALL), epoch_num)
print(output_data)


print("CS Sampling and Reconstruction by DrTransNet End")
